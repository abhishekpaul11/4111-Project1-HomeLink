...
def display_apartment(apt_info):
    st.markdown(
        """
        <style>
        .box {
        ...
        if key_to_be_displayed is not None:
            ...
            if key == 'manager_name':
                if owner_id != manager_id:
                    ...
                    st.markdown(f'<div class="sub-field"><span class="key">MANAGER DETAILS</span><span class="value"><i>Self-Managed</i></span></div></div>', unsafe_allow_html=True)
